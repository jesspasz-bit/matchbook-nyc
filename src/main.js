...
window.openRefresh = function(spotId) {
...
window.toggleLike = async function(findId) {
  ...
  if (existing) {
    ...
    await supabase.from('likes').delete().eq('find_id', findId).eq('user_id', currentUser.id);

  } else {

    await supabase.from('likes').insert({ find_id: findId, user_id: currentUser.id });
...
window.toggleFollow = async function(otherId, isFollowing) {
  ...
  if (isFollowing) {
    ...
    await supabase.from('follows').delete().eq('follower_id', currentUser.id).eq('following_id', otherId);

  } else {

    await supabase.from('follows').insert({ follower_id: currentUser.id, following_id: otherId });
...
function openAddModal() {
  ...
  modal.innerHTML = `
    ...
    <div class="modal-content">
      ...
      <input type="text" id="spot-name" placeholder="Place name (e.g. Bemelmans Bar)" />

      <input type="text" id="spot-address" placeholder="Address or neighborhood" />

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">

        <input type="number" id="spot-lat" step="0.0001" placeholder="Latitude" />

        <input type="number" id="spot-lng" step="0.0001" placeholder="Longitude" />

      </div>

      <button class="btn-ghost" id="use-location" style="width:100%;margin-bottom:8px;">📍 Use my current location</button>
  ...
  document.getElementById('use-location').onclick = () => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      document.getElementById('spot-lat').value = pos.coords.latitude.toFixed(4);

      document.getElementById('spot-lng').value = pos.coords.longitude.toFixed(4);

    }, () => alert('Could not get your location. Type the coordinates manually.'));
